            ...
            for i, (c, r) in enumerate(zip(no_bulge_offtarget_sequence, target_seq)):
                ...
                if r == '-':
                    if 0 < k < len(target_seq):
                        ...
                        dwg.add(dwg.rect((x, box_size * 1.4 + y), (box_size * 0.6, box_size * 0.6), opacity=0.5,
                                         fill=colors[c]))
                    ...
                    dwg.add(dwg.rect((x, box_size + y), (box_size, box_size), opacity=0.5, fill=colors[c]))
                    dwg.add(dwg.text(c, insert=(x + 3, 2 * box_size + y - 3), fill='black',
                                     style="font-size:15px; font-family:Courier"))
                    k += 1
...
def visualizeOfftargets(infile, outfile, title, PAM):
    ...
    if len(offtargets) == 0:
        logger.info("No off-targets in the input file")
        exit()
...
infile = "/groups/clinical/projects/Assay_Dev/changeseq_be/CSB_04/identified/ASS1_ABE_rep1_identified_matched_annotated.csv"
outfile = "/groups/clinical/projects/Assay_Dev/changeseq_be/CSB_04/visualization/ASS1_ABE_rep1_offtargets.svg"
